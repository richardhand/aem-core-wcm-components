...
;(function(h, $){
    ...
    var tcExecuteBeforeTest = new TestCase("Setup Before Test")
        ...
        .execFct(function (opts, done){
            c.addComponent(c.rtFormButton_v2, h.param("testPagePath")(opts)+c.relParentCompPath,"cmpPath",done)
        })
    ...
    var checkDefaultButtonAttributes = new h.TestCase("Check Default Button Attributes", {
        ...
        .asserts.isTrue(function () {
            return h.find(".cmp-form-button__button[type='submit']", "#ContentFrame").size() == 1
        })
        //check that the title on the button should be "Submit"
        .asserts.isTrue(function () {
            return h.find(".cmp-form-button__button", "#ContentFrame").text().trim() == "Submit"
        });
    ...
    var createButton = new h.TestCase("Create a Button",{
        ...
        .asserts.isTrue(function () {
            return h.find(".cmp-form-button__button[type='Button']","#ContentFrame").size() == 1
        });
    ...
    var setButtonText = new h.TestCase("Set Button Text",{
        ...
        //Check if the button tag is rendered with the correct type
        .asserts.isTrue(function() {
            return h.find(".cmp-form-button__button","#ContentFrame").text().trim() == "Test Button"
        });
