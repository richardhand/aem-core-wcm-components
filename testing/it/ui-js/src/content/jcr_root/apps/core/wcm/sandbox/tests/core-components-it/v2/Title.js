...
;(function(h, $){
    ...
    var tcExecuteBeforeTest = TestCase("Setup Before Test")
        ...
        .execFct(function (opts, done){
            c.addComponent(c.rtTitle_v2, h.param("testPagePath")(opts)+c.relParentCompPath,"cmpPath",done)
        })
    ...
    var checkExistenceOfTypesUsingPolicy = new h.TestCase("Check available title types defined in a policy",{
        ...
        .execFct(function (opts,done) {
            ...
            c.createPolicy("/title/v2/title/new_policy",data,"policyPath",done)

        })

        .execFct(function (opts,done) {
            var data = {};
            data["cq:policy"] = "core/wcm/sandbox/components/title/v2/title/new_policy";
            data["sling:resourceType"] = "wcm/core/components/policies/mapping";

            c.assignPolicy("/title/v2/title",data,done)
     ...
     */
    new h.TestSuite("Core Components - Title v2", {path:"/apps/core/wcm/sandbox/tests/core-components-it/v2/Title.js",
