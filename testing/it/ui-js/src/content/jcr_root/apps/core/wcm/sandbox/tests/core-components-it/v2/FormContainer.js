...
;(function(h, $){
    ...
    var tcExecuteBeforeTest = new TestCase("Setup Before Test")
        ...
        .execFct(function (opts, done) {
            c.addComponent(c.rtFormContainer_v2, h.param("testPagePath")(opts) + c.relParentCompPath, "containerPath", done)
        })

        // inside the form add an form text input field
        .execFct(function (opts, done) {
            c.addComponent(c.rtFormText_v2, h.param("containerPath")(opts) + "/", "inputPath", done)
        })
        ...
        .execFct(function (opts, done) {
            c.addComponent(c.rtFormButton_v2, h.param("containerPath")(opts) + "/", "buttonPath", done)
        })
     ...
     */
    var setContentPath = new TestCase("Set Content Path",{
        execBefore: tcExecuteBeforeTest,
        execAfter: tcExecuteAfterTest})
     ...
     * The main test suite.
     */
    new h.TestSuite("Core Components - Form Container v2",{path:"/apps/core/wcm/sandbox/tests/core-components-it/v2/FormContainer.js",
        execBefore:c.tcExecuteBeforeTestSuite,
        execInNewWindow : false})

        .addTestCase(storeContent)
        .addTestCase(setMailAction)
        .addTestCase(setContentPath)
